...
class ActuarialValuationEngine:
    ...
    def _apply_statutory_floors(self, policy_type: str, calculated_reserve: float, capital_03_pct: float, capital_qx: float) -> float:
        ...
        if p_type in ["kredit", "credit", "life_death_single_payment"]:
    ...
    def _generate_cashflow_projection(self, policy: PolicyRecord, projection_date: pd.Timestamp, forward_shift: int = 0) -> Tuple[Dict[str, float], int]:
        ...
        if policy.policy_type.lower() in ["kredit", "credit", "life_death_single_payment"]:
    ...
    def execute_valuation(self, policy: PolicyRecord) -> Dict[str, Any]:
        ...
        if p_type in ["life_endowment", "yigim", "yığım", "endowment", "life_survival_m_payments", "life_survival_single_payment"] or "yığım" in p_type or "yigim" in p_type or "endowment" in p_type or "life_survival_m_payments" in p_type or "life_survival_single_payment" in p_type:
            ...
            return {
                ...
                'Dx_e': float(round(Dx[e], 4)),
                'Nx_e': float(round(Nx[e], 4)),
                'Cx_e': float(round(Cx[e], 4)),
                'Mx_e': float(round(Mx[e], 4)),
